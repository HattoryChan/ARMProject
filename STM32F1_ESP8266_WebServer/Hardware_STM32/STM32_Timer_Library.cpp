...
void TIM2_IRQHandler(void)        
 {         
    //collibrated for 8 Mhz output quartz
...
/*    Initiall output high speed generator
*     
*     
*/
void ClockHSETry()
{
   RCC->CR|=RCC_CR_HSEON; //Generator start HSE.
   while (!(RCC->CR & RCC_CR_HSERDY)) {}; // Waiting for readiness
   RCC->CFGR &=~RCC_CFGR_SW; //Reset bit's
   RCC->CFGR |= RCC_CFGR_SW_HSE; // Go to HSE      
}

/*    Initiall output high speed generator freq multiplier
*     
*     
*/
void ClockPPLHSETry()
