...
#define I2C_ADDRESS_IAQ 	0b10110100	//0b1011010
...
void InitKeyAll(void)
{
 ...
 // GPIOC->CRH   &=~GPIO_CRH_CNF8; //  ??????????? ????? ?? PC8.
  GPIOC->CRH   |= GPIO_CRH_MODE13; // ???? ?????????? LED3 PC9 ?? ?????.
  GPIOC->CRH   &=~GPIO_CRH_CNF13; // ??????????? ????? ?? PC9.

  GPIOA->CRL&=~GPIO_CRL_MODE0; // ?????? "USER" PA0 - ?? ????.
}
int main(void)
{
	uint8_t TxBuf[10]={0, 0, 0, 0, 0, 1, 1, 1};  
	uint8_t RxBuf[10];
	uint8_t size;
