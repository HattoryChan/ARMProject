...
bool g_UnreadMess = false;  //Show when we have unread message, true - have unread
...
/*  Wait for a responce SecCount second, if don't have - return error.
*   Input: SecCount - waiting second value
*   Output: '0' - error, '1' - we have answer
*/
bool ESP8266_WaitInMess(uint8_t SecCount)
{   
   uint32_t sec = SecCount;    //wait seconds number
 for (; sec != 0; sec--)
	{
		for(int i = 0x5B8D80; i !=0; i--);										// 1 second delay if Fmax = 72 Mhz
      {
       if(g_UnreadMess  == true)
         return 1;                        //We have message
      }
	}
...
int AT_CheckReady()
{
   ...
   if(ESP8266_WaitInMess(10) == 1)  //We have input mess
   {
   ...
   switch(Mode)
   {
      case 1:                       //Wi-fi client
         if(SaveToFlash == false)
         {
            ...
            if(ESP8266_WaitInMess(10))  //We have input message
            {
               if(ESP8266_SearchStringBuffer("OK") != 513)  //And in mes we have "OK"                 
                  return 1;     //Operation succesful
               else 
...
/*  Send "AT+CIPMUX= " to ESP8266, answer "OK"
...
/* Create wi-fi SoftAP
*   "AT+CWMODE" must be "2"(SoftAp)
*  Input: SSID - created ssid name
*         Pass - created ssid password          
*  Output: 0 - error esp8266, 1 - Successful, 
*          2 - another error
*/
int AT_WifiCreated(char* SSID, char* Pass)
{
  ESP8266_RepealArrayWritePos();    //Repeal write pos
  ESP8266_NullInBuffer();       //Null input buffer
   
      //Comand structure: "AT+CWSAP="SSID","PASS",<chl>,<enc>\r\n
  ESP8266_SendStr("AT+CWSAP_CUR=\"");    //Send command
  ESP8266_SendStr(SSID);                  //Send SSID   
  ESP8266_SendStr("\",\"");     
  ESP8266_SendStr(Pass);                  //Send pass  
  ESP8266_SendStr("\",1,4");             //chanel and encryption method 
  ESP8266_SendStr("\r\n");              // End command 
   
  ESP8266_WaitInMess(20);             //Wait input mess
...
*  answer - "OK", 
*  Input: Mode - '0' - normal transmission mode,
*                '1' - UART-WIFI passthrough mode
*  Output:  '0'- error esp8266, '1' - Successful, '2' - another ERROR
*/
int AT_TransmissMode(uint8_t Mode)
{
  ESP8266_RepealArrayWritePos();    //Repeal write pos
  ESP8266_NullInBuffer();       //Null input buffer
   
   //Comand structure: "AT+CIPMODE=<Mode>\r\n
   ESP8266_SendStr("AT+CIPMODE=");
   ESP8266_Send(Mode+48);
   ESP8266_SendStr("\r\n");  
