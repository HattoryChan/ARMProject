/* Header for ESP8266_AT.cpp((NOT USED)
*
*
*
*
...
int ESP8266_SearchStringBuffer(char* SearchTwoSymb, int StartSearchPos); //Search Input string in ESP8266 AT input buffer from the starting position

int ESP8266_SearchSymbolBuffer(char SearchSymb); //Search symbol in ESP8266 AT input buffer 
int ESP8266_SearchSymbolBuffer(char SearchSymb, int StartSearchPos); //Search symbol in ESP8266 AT input buffer 
...
bool ESP8266_WaitInMess(uint8_t SecCount); //Wait for a responce 10 second, if don't have - return error.
...
int AT_WifiCreated(char* SSID, char* Pass); //Create wi-fi SoftAP
int AT_TransmissMode(uint8_t Mode); //Set transmission mode
